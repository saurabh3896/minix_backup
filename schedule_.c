...
static void balance_queues(struct timer *tp)
{
	...
	for (proc_nr=0, rmp=schedproc; proc_nr < NR_PROCS; proc_nr++, rmp++) {
		...
		else if (rmp->deadline == INT_MIN) {
			/* schedule using standard RR policy */
			printf("The submitted deadline is out of range, please enter a valid deadline for this process.\n");
			schedule_process_local(rmp);
		}
		else {
			if (rmp->flags & IN_USE) {
